...
list_month = ['Whole year'] + list_month #to add the "whole year" option
list_hour = df['hour'].unique().tolist()

#Show a default graph when opening the dash app we create a default df for Hollywood in May 2020 between 2 & 8 am:
df_default = df[df['year'] == 2020].reset_index(drop=True)
df_default = df_default[df_default['month_name'] == 'May'].reset_index(drop=True)
df_default = df_default[df_default['AREA NAME'] == 'Hollywood'].reset_index(drop=True)
...
def default_pie():
    ...
    title='Repartition of crime type for Hollywood',)
    fig_pie_default.update_traces(textposition='inside', textinfo='percent+label',#to put the label inside the pie
    hovertemplate="<br>".join(["Period: {} {}".format(df_default['month_name'][0],df_default['year'][0]),
        "Hours range: between {}h and {}h".format(start_hour,end_hour),
        "Type of crime: %{label}",
        "Number: %{value}"] ))
    fig_pie_default.update_traces(hole=.4) #change from pie to donut chart
...
def default_bar():
    ...
    xaxis_title="May 2020 between 2h and 8h",
...
def default_fig_line():
    ...
    title= "Crime number per hour between 2h and 8h in May 2020",
...
def default_fig():
    ...
    title="LA's areas crime map for May 2020 between 2h and 8h",
...
app.layout = html.Div([
    ...
    dcc.RangeSlider(min(list_hour),max(list_hour),1,value=[2,8],allowCross=False,
...
def display_hour_range(hour_choice):
    # We return an H5 HTML component with the range of hours selected.
